...
import com.macro.mall.search.enumeration.SortFieldEnum;
...
/*        if(sort==1){
        ...
        String fieldName = SortFieldEnum.getFieldNameByCode(sort);
        if (StringUtils.hasLength(fieldName)) {
            nativeSearchQueryBuilder.withSorts(SortBuilders.fieldSort(fieldName).order(SortFieldEnum.getSortOrderByCode(sort)));
        }
        //按相关度
        nativeSearchQueryBuilder.withSorts(SortBuilders.scoreSort().order(SortOrder.DESC));
